...
def extract_data(cds_df, model, node, parent_mapping_column_list, raw_data_dict):
        ...
        for parent_mapping_column in parent_mapping_column_list:
            if node == parent_mapping_column['node']:
                if parent_mapping_column['property'] in cds_df.keys():
                    ...
                    new_df[parent_mapping_column_name] = cds_df[parent_mapping_column['property']]
    return new_df
...
if args.extract_raw_data_dictionary == False:
    for data_file in glob.glob(path):
        ...
        for node in model['Nodes']:
            ...
            df_dict[node] = extract_data(Participant, model, node, parent_mapping_column_list, raw_data_dict)
